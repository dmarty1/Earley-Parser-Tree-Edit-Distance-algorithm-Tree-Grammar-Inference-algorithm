...
#notes:
#make it [start][end] by potentially flipping "*","S" => "S","*" the grammar and the word
#flip just grammar 
#flip just word
#flip both
#affects on start, end 
    
#trying to to flip the start and end, as well as the grammar 
...
def main(A,S,words,L,setC,S2):
    ...
    def predictorepsilon():
        ...
        for start,end,ns,es in add:
            if end not in S[start]:
                S[start][end] = []
            if not in2(ns,S[start][end]):
                S[start][end].append(ns)
                modify = True
        return modify
        ...
        for start,Ss in S: 
            for end,Sse in Ss:
                for se in Sse:
                    B= target(se)
                    if B==False or B=="@":
                        #for all the end that have the same start
                        '''
                        listOfStarts = []
                        for jj, Sjj in S:
                            for ii, Sii in Sjj:
                            ...
                            for elem in ix:
                                if target(elem)==False:
                                    ...
                                    if ns.it <= len(A[ns.ir][1]):
                                        add.append((end,jj,ns,se))
                                            
                                        
                    
                
        for end,jj,ns,se in add:
            if end not in S[jj]:
                S[jj][end]=[]
            if not in2(ns,S[jj][end]):
                S[jj][end].append(ns)
                modify = True
        return modify
    ...
    def bigbuilder():
        ...
        if startitem==None:
            return None,None
        return builtree(len(words),0,"*",startitem)
